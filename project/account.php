...
if (empty($_GET["action"])||$_GET["action"]=="viewDetail") {
    ...
    if ($acc->chkPassword($_POST["oldPassword"])) {
        ...
        if ($_FILES["photo"]["error"] == 0){
            $h = new SingleImageUploadHandler("photo");
            $h->saveAs(ROOT."/images/userfile/users/{$_SESSION['userId']}/photo128.jpg", 128, 128);
            $h->saveAs(ROOT."/images/userfile/users/{$_SESSION['userId']}/photo64.jpg", 64, 64);
            $h->saveAs(ROOT."/images/userfile/users/{$_SESSION['userId']}/photo32.jpg", 32, 32);
        }
        showMsg("updated");
    } else {
        session_destroy();
