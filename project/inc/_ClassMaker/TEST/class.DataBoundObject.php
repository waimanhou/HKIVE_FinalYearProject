...
abstract class DataBoundObject {
        ...
        if (isset($id)) {
            ...
            if (count($this->_PKFields) == 1 && !is_array($this->_ID)) {
                ...
                if (!is_array($this->_ID)) {
                    die(DEBUG_MODE === true ? "Error! Wrong number of PKs" : "Error occured");
                }

                foreach ($this->_ID as $key => $value) {
                    if (!in_array($key, $this->_PKFields)) {
                        die(DEBUG_MODE === true ? "Error occured, the key `$key` is not found in the PK Fields list of {$this->_tableName}" : "Error occured");
                    }
                }
            }
        }
    ...
    public function searchf($args) {
        ...
        foreach ($args['field_list'] as $k => $v) {
            ...
            if (is_array($v[1])) {          //Case 4 BEGIN
                ...
                if (!is_null($v[1])) {
                    $SqlWhere.="and `$k` {$v[0]} \"" . addslashes($v[1]) . "\" ";    //Case 2,3
                } else {
                    if ($v[0] == '=') {
                        $SqlWhere.="and `$k` is null ";      //Case 5
                    } else {
                        $SqlWhere.="and `$k` is not null ";   //Case 6,7
                    }
                }
            }
        }
    ...
    public function __toString() {
        ...
        $v.="] ";
        foreach ($this->_relationMap as $key => $value) {
            $v.="$value = {$this->$value}, \t"; //"$value." = ".$this->$value ."\t";"
        }
        $v.="\n";
        return $v;
    }
